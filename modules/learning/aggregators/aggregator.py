...
class Aggregator:
    ...
    def set_nodes_to_aggregate(self, nodes_to_aggregate: List[str]) -> None:
        ...
        if not self.__finish_aggregation_lock.locked():
            ...
            self.__finish_aggregation_lock.acquire(
                timeout=get_nested_value(self.engine_config, Keys.AGGREGATION_TIMEOUT)
            )
        else:
            raise Exception(
    ...
    def add_model(
        ...
        if self.__waiting_aggregated_model and self.__models == {}:
            if set(contributors) == set(self.__train_set):
                self.log.info(self.node_name, "Received an aggregated model.")
                self.__models = {}
            ...
            if len(self.__train_set) > len(self.get_aggregated_models()):
                ...
                self.log.debug(self.node_name, "Received a model when is not needed.")
            self.__agg_lock.release()
        return []
    ...
    def wait_and_get_aggregation(
        ...
        if n_model_aggregated != len(self.__train_set):
            ...
            self.log.info(self.node_name, "Aggregating models.")
